...
namespace Microsoft.MixedReality.WebRTC.Unity
{
    ...
    public abstract class CustomVideoSender<T> : VideoTrackSource where T : class, IVideoFrameStorage, new()
    {
        ...
        /// </summary>
        public ExternalVideoTrackSource ExternalSource { get; private set; }
        ...
        protected override Task CreateVideoTrackSourceAsync()
        {
            ...
            {
                ExternalSource = ExternalVideoTrackSource.CreateFromI420ACallback(OnFrameRequested);
            }
            else if (typeof(T) == typeof(Argb32VideoFrameStorage))
            {
                ExternalSource = ExternalVideoTrackSource.CreateFromArgb32Callback(OnFrameRequested);
            }
            else
            {
            ...
            //Track = LocalVideoTrack.CreateFromExternalSource(trackName, ExternalSource);
            //if (Track == null)
            //{
        ...
        protected override void DestroyVideoTrackSource()
        {
            base.DestroyVideoTrackSource();
            if (ExternalSource != null)
            {
                ExternalSource.Dispose();
                ExternalSource = null;
            }
        }
