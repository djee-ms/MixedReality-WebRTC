...
mrsResult OpenVideoCaptureDevice(
    ...
    rtc::scoped_refptr<webrtc::VideoCaptureModule>& vcm_out) noexcept {
  vcm_out = nullptr;
#if defined(WINUWP)
  ...
  for (const auto& id : device_ids) {
    vcm_out = webrtc::VideoCaptureFactory::Create(id.c_str());
    if (vcm_out) {
      return Result::kSuccess;
    }
  }
...
mrsResult MRS_CALL mrsPeerConnectionAddLocalVideoTrack(
  ...
  *trackHandle = nullptr;
  ...
  auto res = OpenVideoCaptureDevice(config, vcm);
  if (res != Result::kSuccess) {
...
//    return Result::kUnknownError;
//  }
    rtc::scoped_refptr<webrtc::VideoTrackInterface> video_track = nullptr; // TODO
